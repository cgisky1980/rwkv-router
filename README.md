...
自进化的本地优先智能路由组件，从 Ai00-X 桌面客户端中抽离的独立项目。斯坦福 & Together AI 的《Intelligence per Watt》证实：**88.7% 的 AI 查询可由本地模型处理**，按置信度做端云分配即可。RWKV-Router 是这一路线的完整开源实现——并且多出文章方案没有的第三块：**本地纯 Rust 自进化闭环**。
